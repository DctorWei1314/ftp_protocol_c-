...
int Server::listenClient() {
    ...
    while(true) {
        ...
        //sendMessage("qewtttttttttttttttttt");
        cout<<"getmessage"<<"cmd:"<<cmd<<"arg:"<<arg<<endl;
        if(cmd == "SYST") {
        ...
        if(cmd == "PASV") {
            ...
            pasvPort = setPasv();
            //sendMessage("test");
//            cout<<"TEST0:"<<sendMessage("test")<<endl;
            pasvArg2 = pasvPort % 256;
            pasvArg1 = pasvPort / 256;
            cout<<"pasvPort:"<<pasvPort<<endl;
            pasvIp = localIp;
            int loc = pasvIp.find('.');
            while(loc>=0) {
...
//            cout<<"TEST1:"<<sendMessage("238 uihoofeahytehrgfdsfghjkjyrneb")<<endl;
            int dataAddrLen = sizeof(dataAddr);
//            string sen = "227 Entering Passive Mode (" + pasvIp +", " + to_string(pasvArg1) + "," + to_string(pasvArg2) + ").";
//            int tttt = sendMessage("227 Entering Passive Mode (" + pasvIp +
//                        ", " + to_string(pasvArg1) + "," + to_string(pasvArg2) + ").");
//            int tttt =sendMessage(sen);
            sendMessage("228 Entering Passive Mode (" + pasvIp +
                        ", " + to_string(pasvArg1) + "," + to_string(pasvArg2) + ").");
//            cout<<"TTTT:"<<tttt<<endl;
//            cout<<"TEST2:"<<sendMessage("test")<<endl;
            cout<<"227 Entering Passive Mode (" + pasvIp +
                  ", " + to_string(pasvArg1) + "," + to_string(pasvArg2) + ")."<<endl;
        ...
        if(cmd == "MKD") {
            ...
            string abspwd;
            abspwd = rel2abs(pwd);
            QString newDir = QString::fromStdString(abspwd+"/"+arg);
            cout<<"newDir:"<<newDir.toStdString()<<endl;
            QDir *temp = new QDir;
                bool exist = temp->exists(newDir);
                if(exist){
                    sendMessage("250 Dir exist!!!");
                }
                else
                {
                    bool ok = temp->mkdir(newDir);
                    cout<<newDir.toStdString()<<endl;
                    if( ok )
...
//            cout<<"vvvvv"<<endl;
            ...
            string abspwd;
            abspwd = rel2abs(pwd);
            QString fr = QString::fromStdString(abspwd+"/"+arg);
            sendMessage("350 get the filename to be renamed!");
            ret = recvStr();
            if(ret<0) {
                Sleep(50);
                continue;
            }
//            cout<<"vfr"<<fr.toStdString()<<endl;
            if (cmd != "RNTO"){
                sendMessage("250 should be RNTO command");
                continue;
            }
            QString to = QString::fromStdString(abspwd+"/"+arg);
//             cout<<"vto"<<to.toStdString()<<endl;
            if(QFile::rename(fr,to))
            sendMessage("250 rename successfully!");
            else sendMessage("250 rename failure!");
        }
        ...
        if(cmd == "DELE") {
            string abspwd;
            abspwd = rel2abs(pwd);
            QString f = QString::fromStdString(abspwd+"/"+arg);
            if(QFile::remove(f)){
                sendMessage("250 delete suceecessfully.");
            }else {
                 sendMessage("250 delete failure.");
            }
            continue;
        }
        if(cmd == "SITE") {
