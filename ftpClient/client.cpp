...
int Client::recvControl(int stateCode, string errorInfo) {
    ...
    if(nextInfo.size()==0) {
        ...
        int infolen;
        while(true){
             infolen = recv(controlSocket, buf, BUFLEN, 0);
             if(stateCode == 228){
                 cout<<"ggg"<<infolen<<endl;
...
//    FILE* ifile = fopen(localName.c_str(), "rb");
    QFile ifile(QString::fromStdString(localName));
//    infoThread->sendInfo(localName);
...
//        count = fread(databuf, 1, DATABUFLEN, ifile);
        count = ifile.read(databuf, DATABUFLEN);
